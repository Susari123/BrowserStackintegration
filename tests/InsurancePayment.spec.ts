...
test.only('Billing - Insurance payment', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Search insurance plan' }).click();
  ...
  await page.getByRole('textbox', { name: 'Search insurance plan' }).fill('aetna');
  // await expect(page.getByRole('heading', { name: 'AETNA PLUS' })).toBeVisible();

  await page.getByLabel('Sidebar', { exact: true }).locator('div').filter({ hasText: /^AETNA PLUS$/ }).click();
  ...
  await page.locator('textarea').fill('Insurance Payment');
